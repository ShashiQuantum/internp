...
<nav id="mainNav" class="navbar navbar-default navbar-custom navbar-fixed-top" style="background-color:white; border-bottom: 3px solid #DCEDC8;">
 <div style="margin-left: 10px;float:left;position: relative;width:75px;"></div>
  <div class="container">
	 ...
	 <a href="<?php echo base_url(); ?>quota"><img src="<?php echo base_url(); ?>public/img/logos/quantum-logo-big.png" style="height:45px; width:100px:float:left"/></a>
   </div>
  </div>
</nav>
...
<center><h3><u>Overall Quota Report</u></h3> </center>

<?php
$qset = $this->session->userdata('pid'); //current qset_id
$centre =$this->session->userdata('ccode'); //centre code 
if($qset!='' && $centre !='')
{

$table=$this->MHabitat->getPTable($qset);
$filter =" AND resp_id in (select distinct resp_id from $table WHERE centre = $centre ) ";

$sqlq="SELECT qid from project_quota WHERE qset = $qset ;";
...
foreach($qdata as $qd)
{
	...
	if($qopt)
        foreach($qopt as $q)
        {
                ...
                $term=$q->term;
		//$total=$total+$value
                $vopt=$this->MHabitat->get_cod_filter($table,$term, $value, $filter);
		$total = $this->MHabitat->get_cod_ov($table,$term, $filter );
		$tot =$total;
		$str1.="<td>$ot </td>";
		$str2.="<td>$vopt</td>";
		$tp=($vopt * 100 / $total);
...
<!------------------------------------------------------- GRAPH CHART --------------------------------------------------------------->
  <script type="text/javascript">
	...
	foreach($optxt as $d)
	{
		...
		$value=$d->value;
		$term=$d->term;
		$count_opt=$this->MHabitat->get_cod_filter($table,$term, $value, $filter); //counting current term options as per value
		$cov=$this->MHabitat->get_cod_ov_filter($table,$term, $filter ); //counting overall current term
		if($i==1)
		$ab="#B82E2E";
		if($i==2)
		$ab="#3366CC";
...
<div style="width: 95%; height: 65%; padding: 5px;float: left; margin-left: 3px; border-style: groove;">

 <div id="c<?php echo $qid; ?>" style="height:300px; width:510px;"></div> 
<!--
<div id="p<?php echo $qid; ?>" style=" height:100px; width:410px; float: right; margin-top:5%;"></div>
-->
<?php
 echo "$str1 $str2 $strp2";
//echo "$strp2 ";
?>
</div>
<!------------------------------------------------------- END OF GRAPH CHART --------------------------------------------------------------->
