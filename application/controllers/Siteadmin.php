        ...
        {       //print_r($_POST);
			...
			$login=$this->session->userdata('sauserid');
		       ...
		       $pcat=4; 
	               ...
	               $this->load->model('MProject');
                       $chkp=$this->MProject->getDataBySql("select * from project where name='$pn'");
                       if(!$chkp)
                       {
			 ...
			 $dtn.='_'.$pid;
                         if($pid)
                         {
                                 ...
                                 $rdd2=$this->MProject->doSqlDML("CREATE TABLE IF NOT EXISTS $dtn (id int(10) primary key AUTO_INCREMENT, resp_id bigint(20) not null, q_id int(5) not null, r_name varchar(30) not null, mobile varchar(11) not null, centre varchar(6),timestamp varchar(20), c_area varchar(4) not null, c_sub_area varchar(4) not null,  i_name varchar(30) not null, i_mobile varchar(11) not null, i_date datetime, e_date datetime,address varchar(40), o_sec varchar(1) not null, n_sec varchar(1) not null, gender varchar(1) not null, week_day char(1)  null, age varchar(2) not null, st varchar(2) not null, age_yrs varchar(2) not null, store varchar(2) not null, product varchar(2) not null, ref_no varchar(15) not null, latt varchar(30) null, lang varchar(30) null) ENGINE=InnoDB  DEFAULT CHARSET=latin1 COLLATE=latin1_general_ci ");
				 if($rdd2)
                                 {
              ...
              $mt= $this->input->post('mt'); 
                ...
                if($mt == 1){
           	   ...
           	   $config['upload_path'] = $ff= dirname($_SERVER["DOCUMENT_ROOT"])."/htdocs/vcimsweb/uploads/image/";
                   //$config['upload_path'] = $ff= '/quantumdigiadmin/vcimsweb/uploads/image/'; 
