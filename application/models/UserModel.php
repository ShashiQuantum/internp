...
class UserModel extends CI_Model{
    ...
    {        
        $res=$this->db->insert('web_user',$udata);
        $res1=$this->db->insert('vcims_user_info',$uinfo);
        ...
        public function update_user_details($user=null, $details=array())
	{
		...
		{
			$field = 'user_id';
			$data = $this->db->update('user_info', $field, $user, $details);
			if($data)
			{
				return true;
			}
	...
	public function get_user_by_hash($hash=null)
	{
		...
		{
                        $this->db->select('user_id');
                        $query=$this->db->get_where('web_user',array('hash' => $hash));
                        $result=$query->row();
                        if($result)
                         return $result->user_id;
                        else
	...
	public function reset_pass($user=null, $pass=null)
	{
		if($user && $pass)
		{
			...
			$pwd = $this->hashMake($pass, $salt);
			$this->db->where('user_id', $user); 
			$update = $this->db->update('web_user',array('pass' => $pwd, 'status' => 1, 'salt' => $salt));
			return true;
		}
		return false;
	}
...
/*
	...
	{
		if($this->db->insert('admin_user',$fields))
