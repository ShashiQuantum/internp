...
class MSausermodel extends CI_Model{
	...
	public function reset_pass($user=null, $pass=null, $type )
	{ 	//echo "user: $user, p: $pass, t: $type ";;
		if($user && $pass)
		{
			$timezone = "Asia/Calcutta";
			if(function_exists('date_default_timezone_set')) date_default_timezone_set($timezone);
			...
			if($type == 2)
			{
                         ...
                         $update = $this->db->update('vpms.vams_employee',array('password' => $pwd, 'status' => 1, 'salt' => $salt));
			}
			else
			{
			$this->db->where('user_id', $user); 
        ...
        {
                $this->db->select('*');
                $query=$this->db->get_where('user_permission',array('prm_id' => $id));
                $results=$query->row();
                if($results)
