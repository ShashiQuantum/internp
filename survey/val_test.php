...
<tr style="height:50%;"><td><center>
        ...
        <tr style=height:50px;color:black;><td></td><td>Thank you for making Uber Eats better!</td></tr>

        <tr style="height:10px;"><td>Gender <font color="red">* </font> </td><td><select name="gender" id="gender" required><option value=''>SELECT YOUR GENDER</option><option value=1>Male</option><option value=2>Female</option><option value=3>Prefere not to answer</option></select></td></tr>
        <tr style="height:10px;"><td>Age <font color="red">*</font> </td><td><select name="age" id="age" required><option value=''>SELECT YOUR AGE</option><option value=1>Below 18 years</option><option value=2> 18 - 25 years</option><option value=3>26 - 35 yrs</option><option value=4> 36 - 45 yrs</option><option value=5> 46 years and above</option><option value=6>Prefer not to answer</option></select></td></tr>
	...
	<tr style="height:10px;"><td>Highest Education Level <font color="red">*</font> </td><td><select name="edu" id="edu" required><option value=''>SELECT YOUR HIGHEST EDUCATION</option><option value=1>Schooling upto 12th </option><option value=2>Some college but not graduate </option><option value=3>Graduate / Post graduate (General courses like BA, MA, B.Sc. M.Sc. etc.)</option><option value=4>Grad/Post graduate (Professional courses like B.Tech, M.Tech, MBA etc.) </option></select>
      <?php 
        ...
        ?>
	<tr><td>Select City<font color="red">*</font></td><td><select name='centre' id=="city" onchange="select(this.value);"  required><option value=''>SELECT YOUR CITY</option> <?php if($dc1->count()>0)foreach($dc1->results() as $d){ echo "<option value=$d->centre_id>$d->cname</option>";}?></option> <option value='-1'>Others</option></select><br> <br><div style="height:10px;width:100%;display:none;" id="oc"> <input type='text' name='city' size='40' id='ocity' placeholder='Enter your city if not in above list'></div><span class='error'><p id='oc_error'></p></span> </td></tr>
       <!-- <tr  style="height:10px;width:100%;display:none;" id="oc"><td>Other City here <font color="red"> * </font> </td><td><input type="text" name="city" id="ocity" size="40" placeholder="Enter your city if not in above list"> <span class="error"><p id="oc_error"></p></span></td></tr>
-->
...
-->       
 <tr style=height:50px;color:lightgray;><td></td><td><input type="submit" name="subinfo" values="Next" style=background-color:#004c00;color:white;height:30px;></td></tr>
...
<script type="text/javascript">
 ...
 <script type="text/javascript">
  document.getElementById('gender').selectedOptions.value = "<?php echo $_POST['gender'];?>";
