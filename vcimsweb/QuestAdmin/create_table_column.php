...
if(isset($_POST['tn_submit']))
{
  ...
  $qset=$_POST['qset'];
   ...
   if($pn!=0)
   {
   $pnn=DB::getInstance()->query("SELECT * FROM `project` WHERE `project_id`=$pn");
   $tn=$pnn->first()->name;

  $str=$tn."_".$qset;

  $ctb=DB::getInstance()->query("CREATE TABLE IF NOT EXISTS $str  (id int(5) primary key AUTO_INCREMENT,resp_id int(5) not null, q_id int(5) not null, r_name varchar(30) not null, mobile varchar(11) not null,sec_$qset varchar(1) not null, gender_$qset varchar(1) not null, week_day_$qset char(1) not null, age_$qset varchar(2) not null, age_yrs_$qset varchar(2) not null, store_$qset varchar(1) not null, product_$qset varchar(1) not null)");

   $pnn=DB::getInstance()->query("UPDATE `project` SET  `data_table`='$str' ,`status`=1 WHERE `project_id`=$pn");
...
if(isset($_POST['trn_submit']))
{
  ...
  $ctb1=DB::getInstance()->query("ALTER TABLE $tbl ADD $trn $dtype($dsize) not null");
  echo "<font color=red>$trn added successfully</font>";
}
?>
...
else
{
 ...
 echo "<br><center><a href=?&st=1>Create New Table</a> &nbsp;&nbsp;<a href=create_table_column.php?&st=2>Create New Term</a></center><br>";
