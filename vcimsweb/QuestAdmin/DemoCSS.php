                                ...
                                if($df->count() > 0)
                                {
                                    foreach ($df->results() as  $val) 
                                    {
                                        ...
                                        if($qqtype=='instruction' || $qqtype=='top3rankcompany')
                                            continue;
                                        else if($qqtype=='text' || $qqtype=='tetarea' || $qqtype=='radio'|| $qqtype =='rating' || $qqtype=='calculated')
                                        ...
                                        {   
                                            ...
                                            {
                                                //update code here
                                                $stat="UPDATE $arr_table[0] SET $arr_column[0]='$sd' WHERE resp_id=$resp_id AND q_id=$qset AND $arr_column[0]!=''";
                                                DB::getInstance()->query($stat);
                                            }
                                            else
                                            {
                ...
                {    
                    ...
                    {   //echo $p->q_id;    
                                ...
                                //find the selected option by user from database
                                        ...
                                        $qtd="SELECT  `q_type` FROM `question_detail` WHERE `q_id`=$qid";
                                        $qtd1=DB::getInstance()->query($qtd);
                                        if($qtd1->first()->q_type=='radio' || $qtd1->first()->q_type=='text' || $qtd1->first()->q_type=='textbox'|| $qtd1->first()->q_type=='textarea' || $qtd1->first()->q_type=='checkbox' || $qtd1->first()->q_type=='calculated')
                                        if($qtd1->first()->q_type!='instruction' || $qtd1->first()->q_type!='top3rankcompany')
                                        {  
                                                //to find table and its column search
                                                $column='';$table='';
            ...
            echo '<br>sec:'.$qsec;
                	...
                	if($sh)
                        ...
                        if($qt=='calculated')
                        {
                                ...
                                echo "<table>";
                                echo "<tr bgcolor=lightgray><td>No. of Pack Consumed</td><td>Pack size</td><td>Total (wt. in grams/ml)</td></tr>";
                                echo "<tr><td width: 5%;><input type=number min=0 name=tp id=tp value=$str1 placeholder='enter total pack in a year'></td><td><input type=number min=0 name=ps id=ps value=$str2 placeholder='enter a pack size'></td><td><input type=text name=tw id=tw style='box-shadow: 0 0 5px 1px #969696;background-color: #ffffff;' value='$str3' placeholder=Total readonly></td></tr>";
                                echo "</table>";
                                 //echo "Number of Pack Consumed : <input type=number min=0 name=tp id=tp value=$str1 placeholder='enter total pack in a year'> * ";
                                 //echo "Pack size : <input type=number min=0 name=ps id=ps value=$str2 placeholder='enter a pack size'> = ";
                                 //echo "Total Consumption : <input type=text size=5 name=tw id=tw value='$str3' placeholder=Total readonly>grams/ml";        
                        }
                        if($qt=='sec')
                        {
                            /*
