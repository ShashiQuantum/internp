...
<html>
<head><title>Delete Project Sequence</title></head>
<body>
<center>
<br><br><h2>Delete Project Sequence</h2><br><br>

<?php
    
	if(isset($_POST['delete_qs']))
	{
	    $pid=$_POST['pn'];
	    $qset=$_POST['qset'];
	    ...
	    if($pid!='0' && $qset!='0')
	    {   
	        $pp=DB::getInstance()->query("DELETE FROM `question_sequence` WHERE `qset_id`= $qset");        
	        echo '<font color=red>Question Sequence deleted sucessfully of qsetid: $qset </font>';        
	    }else
...
<tr><td>QuestionSet ID </td><td><select name="qset" id="qset" onchange="displib4();"><option value="select">--Select--</option> </select> </td></tr>
                    <tr><td colspan="2"><center><input type="submit" name="delete_qs" onclick="return confirm('Are you sure?');" value="Delete Sequence"></center></td></tr>                
                </form>
        </table>          
