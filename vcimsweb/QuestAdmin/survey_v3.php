...
if($_GET['next']==3)
{
        ...
        $qqq="DELETE FROM  `pilot` WHERE  `resp_id` =$resp_id AND  `r_age` ='' ";
...
if(isset($_POST['submit']))
{
	...
	echo '<br>Thank You! Submitted';
        if($qset==16)
           {  $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_16.php?qset=$qset&yn=0";}
        if($qset==17)
           {  $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_9.php?qset=$qset&yn=0";}
        if($qset==23)
           { $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_23.php?qset=$qset";}
        if($qset==11)
           { $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_mystery.php?qset=$qset&yn=0";}
        if($qset==13)
           { $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_13.php?qset=$qset";}
        if($qset==14)
           { $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_14.php?qset=$qset";}
        if($qset==15)
           { $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/fp_15.php?qset=$qset";}
        //else
          //  {$newURL="https://www.digiadmin.quantumcs.com";    }
...
//start previous button==============================
            ...
            if($_SESSION['resp']!=$resp_id)
            {
                echo 'You have entered wrong URL, pls use your correct URL';
                          ...
                          $psec=$_SESSION['psec'];
                           if($psec==0)
                               header('Location: '."https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=-1&rsp=$resp_id");
                  $qqr="select * FROM `question_routine_detail` WHERE qset_id = $qset and next_qid in ( select q_id FROM `question_routine_detail` WHERE qset_id = $qset and qsec in ($psec) order by `qsequence`,q_id)";         
                ...
                {   
                    $qsec=$_GET['sec'];
                    $rw=DB::getInstance()->query("select * FROM `question_routine_detail` WHERE qset_id = $qset and qsec=$qsec order by `qsequence`,q_id");
                        if($rw){
                        $_SESSION['currq']=$rw->first()->q_id;
                        $currq=$rw->first()->q_id;$prev_c=$rw->first()->q_id;}
                }
                //to find the routine details of current question
                 $aaa="select * FROM `question_routine_detail` WHERE qset_id = $qset and qsec in ($psec) order by `qsequence`,q_id";
                	...
                	if($sh)
                        {
                            ...
                            {  //<a href=https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=1&rsp=$resp_id&sec=".$h->qsec_id."></a>
                                if($h->qsec_id==$psec)
                                    echo "<td><input type=button style=background-color:#00e500;color:white;height:30px; name=".$h->sec_title." value='".$h->sec_title."'> </td>";    
                                else
                                    echo "<td><input type=button style=background-color:green;color:white;height:30px; name=".$h->sec_title." value='".$h->sec_title."'> </td>";
                            }
...
echo "</div>";
       ...
       /*        $db=DB::getInstance()->query("select * FROM `question_routine_detail` WHERE qset_id = $qset and next_qid in ( select q_id FROM `question_routine_detail` WHERE qset_id = $qset and qsec in ($psec) order by `qsequence`,q_id)");
                if($db->count()>0)
                {
        ...
        */                
                        ...
                        $sql="select * FROM `question_routine_detail` WHERE qset_id = $qset and qsec in ($psec) order by `qsequence`,q_id";
                            ...
                            {       $eng_qtitle=$rs2->first()->q_title;
                               ...
                               $currq=$cq->q_id;
                                ...
                                {   
                                  ...
                                  echo '<table><tr><td width=4%></td><td colspan=2><font color=blue> '.$eng_qtitle.'</font><font color=red><span id='.$error.'></span></font></td></tr>';  
                            ...
                            if($da->count()>0)
                            {
                                ...
                                {   $str='';
                                    ...
                                    {   
                                        ...
                                        echo "<td widh=45><input type=radio class=big id=q$currq name=q$currq $str value=$s> </td>";
                                    }
                                    if(in_array($s,$arr_temp))
                                        { 
                                            $str="checked='checked'";
                                    ...
                                    echo "<td> $el </td><td><input type=radio class=big  id=q$currq name=q$currq $str value=$s></td></tr></table>";                                
                                }else if($sv==1)
                                {
                                    echo "<table><tr><td> </td>";
                                    for($s=$sv;$s<=$ev;$s++)
                                    ...
                                    {   
                                        ...
                                        echo "<td width=45><input type=radio class=big id=q$currq name=q$currq $str value=$s> </td>";
                                    }
...
OR all_prod_exposed !=  '')");
   ...
   //         	}
                               echo '<br><br><br>';
		  echo "<div id=footer>";
					//echo "<center><a href='https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=3&rsp=$resp_id'><input type='button' name='reset_survey' value='Reset Survey' style=background-color:#004c00;color:white;height:35px;></a> <a href='https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=2&rsp=$resp_id'><input type='button' name='prev' value='Previous' style=background-color:#004c00;color:white;height:35px;></a> <a href='?qset=$qset&next=1&rsp=$resp_id'><input type='submit' name='save' value='Save & Next' style=background-color:#004c00;color:white;height:35px;></a>";

      echo "<a href='https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=2&rsp=$resp_id'><input type='button' name='prev' value='Previous' style=background-color:#004c00;color:white;height:35px;></a>";

echo "<a href='https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=1&rsp=$resp_id#'> <input type='submit' name='save' value='Save & Next' onclick='return Validate();' style='background-color:#004c00;color:white;height:35px;position:absolute; left:50%;'></a> </center> ";
...
echo "</div>"; 
            ...
            echo "<br><br>";
                        ...
                        echo "<div id=footer>";
                                echo "<center><a href='https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=1&rsp=$resp_id'> <input type='button' name='next' value='Continue Survey' style=background-color:#004c00;color:white;height:35px;></a></center>"; 
                        echo "</div>";
                }}}
                else
                {
                ...
                echo 'You have entered wrong URL, pls use your correct URL';
                        ...
                        //start===================================================================================================================================
                                ...
                                {   
                                    ...
                                    if( $q==60)
                                    {
                                           // $newURL="https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=1&rsp=$resp_id&sec=$qsec&msg=1";
                                           //header('Location: '.$newURL);  
                                        //echo "<font color=red>Please fill all the questions to be proceed...</font>";
                                    }
                                    else
                                    {
                                    $sql1="SELECT distinct q_id FROM `question_routine_detail` WHERE qsec in ($qsec) AND qset_id=$qset order by `qsequence`,q_id";
...
//  if($_REQUEST['msg']==1){ $_REQUEST['msg']=0;
//echo('<font color=olive size=2>Please fill again the responses of below these questions to continue...</font>');
//  }
                $arr_nsec=array();
                $qsecs=0;$_SESSION['qt']='';
                $prev_c=$_SESSION['currq'];
                    ...
                    {   //echo '<br>qid: '.$p->q_id;    
                                ...
                                //find the selected option by user from database
                                      ...
                                      $qtd="SELECT  `q_type` FROM `question_detail` WHERE `q_id`=$qid";
                                        ...
                                        if($qtd1->first()->q_type=='radio' || $qtd1->first()->q_type=='text' || $qtd1->first()->q_type=='rating' || $qtd1->first()->q_type=='checkbox'|| $qtd1->first()->q_type=='textarea' || $qtd1->first()->q_type=='checkbox' || $qtd1->first()->q_type=='calculated' || $qtd1->first()->q_type=='sec' || $qtd1->first()->q_type=='dropdown'  )
                                        if( $qtd1->first()->q_type!='instruction' || $qtd1->first()->q_type!='top3rankcompany')
                                        {  
                                                //to find table and its column search
                                                $column='';$table='';
                       ...
                       $arr_getv=array();
                                                  ...
                                                  if($r->count()>0)
                                                    ...
                                                    foreach($r->results() as $rr)
                                                     {
                                                    //echo '<br>sopv::'.$opv;   $getv=$rr->first()->$column;
                                                    $getv=$rr->$column;
                                                    array_push($arr_getv, $getv);  
                                                   //echo '<br>opv::'.$opv.'getv:'.$getv; 
                                                    ...
                                                    $qsec=$rss->first()->qsec;
                                                    $ssarr=explode(",",$psec);
                                                    if($qtt=='textarea' || $qtt=='text' || $qtt=='dropdown' || $qtt=='instruction' )
                                                    {
                                                        array_push($arr_nsec, $qsec);
                                                    }
                            ...
                            //} //end if
                             if( $qtt=='instruction')
                                           {  $sqq="select * FROM `question_routine_detail` WHERE q_id = $nqid and qset_id=$qset";
                                                $rdd=DB::getInstance()->query($sqq);
                                                if($rdd)
                                                {
                                                    $qqsec=$rdd->first()->qsec;
                                                     array_push($arr_nsec, $qqsec);}
                                            }
                	...
                	if($sh1->count()>0)
                        {
                            ...
                            {
                                if($h->qsec_id==$qsec)
                                //https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=1&rsp=$resp_id&sec=".$h->qsec_id.";

                                echo "<td><input type=button style=background-color:#004c00;color:white;height:50px; name=".$h->sec_title." value='".$h->sec_title."'> </td>";    
                                    else
                                echo "<td><input type=button style=background-color:green;color:white;height:50px; name=".$h->sec_title." value='".$h->sec_title."'> </td>";
                            }
                            echo '</tr></table></center><br>';
                        }//end display tab
               */
...
echo "</div>";
                ...
                //echo '<br>a qsec:'.$qsec;        
                        ...
                        //$qq="SELECT distinct `q_id`,`qset_id`, `next_qid`, `op_value`, `qsec`, `routine`, `compare_sign` FROM `question_routine_detail` WHERE `qset_id` = 2 and `qsec` in (11) order by `q_id`"; 
                   /*   
                       $sql="select * FROM `question_routine_detail` WHERE qset_id = $qset and qsec in ($qsec) order by `qsequence`,q_id";
                        $rss3=DB::getInstance()->query($sql);
                     ...
                     */   
                        //print_r($rss);
                       $qlist=array();$qid=0;
                        $qq="SELECT distinct `q_id`,`qset_id` FROM `question_routine_detail` WHERE `qset_id` = $qset and `qsec` in ($qsec) order by  `qsequence`,`q_id`"; 
                        $rss333=DB::getInstance()->query($qq);
                        foreach($rss333->results() as $cq )
                        {  
                            ...
                            {       
                               ...
                               $qt=$rs2->first()->q_type;
                               $currq=$cq->q_id;
                               if($qt!='instruction') if( $qt!='dropdown')
                        ...
                        {    
                               ...
                               if($currq==330)
                               {
                                                if($rss2->count()>0) 
                                         { $str=$rss2->first()->$column;
                                                    echo "<tr height=40><td></td><td>&nbsp;&nbsp;<select  name=text$currq style='font-size:1.2em;'><option>$str</option> <option  value=1>Less than Rs.1000</option><option value=2>Rs.1001-2000</option><option  value=3>Rs.2001-4000</option><option  value=4>Rs.4001-6000</option><option  value=5>Rs.6001-10000</option><option  value=6>Rs.10001-15000</option><option  value=7>Rs.15001-20000</option><option  value=8>Rs.20001-30000</option><option  value=9>Rs.30001-40000</option><option  value=10>Rs.40001-60000</option><option   value=11>Rs.60001-100000</option><option  value=12> More than Rs.100000</option><option  value=13>Refuse</option> </select></td></tr>";
                                          }else echo "<tr height=40><td></td><td>&nbsp;&nbsp;<select  name=text$currq style='font-size:1.2em;'> <option  value=1>Less than Rs.1000</option><option value=2>Rs.1001-2000</option><option  value=3>Rs.2001-4000</option><option  value=4>Rs.4001-6000</option><option  value=5>Rs.6001-10000</option><option  value=6>Rs.10001-15000</option><option  value=7>Rs.15001-20000</option><option  value=8>Rs.20001-30000</option><option  value=9>Rs.30001-40000</option><option  value=10>Rs.40001-60000</option><option   value=11>Rs.60001-100000</option><option  value=12> More than Rs.100000</option><option  value=13>Refuse</option> </select></td></tr>";
                                }    
                               else{
                                        if($rss2->count()>0) 
                                         { $str=$rss2->first()->$column;
...
OR all_prod_exposed !=  '')");
                            ...
                            {   
                                $eng_text=$op->opt_text_value; $opid=$op->op_id; $t=$op->term;$tb='';$arr_temp=array();$str='';$v=$op->value;
                                //echo 'V: '.$v;
                                 //echo "SELECT DISTINCT TABLE_NAME,COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME IN ('$op->term') $currq";
                                $tbf1=DB::getInstance()->query("SELECT DISTINCT TABLE_NAME,COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME IN ('$op->term')");
                                if($tbf1)
                                $tb=$tbf1->first()->TABLE_NAME;
                                $sqtb="SELECT $t FROM $tb WHERE `resp_id`=$resp_id  AND $t IS NOT NULL";
             ...
             echo "<font size=2 color=red><div id=err></div></font>";        
                                 
  echo "<br><div id=footer>";
			//echo "<center><a href='https://www.digiadmin.quantumcs.com/vcims/QuestAdmin/survey_v1.php?qset=$qset&next=3&rsp=$resp_id'><input type='button' name='reset_survey' value='Reset Survey' style=background-color:#004c00;color:white;height:35px;></a>";
