...
a.pid=$pid AND a.fpid IN (SELECT id FROM vcims.app_first_page WHERE type IN ('radio','dropdown','checkbox'));");
		...
		foreach($q->results() as $r){
			...
			if($fpid == 6){
				...
				$temp_arr[ $fpid ] = array('q_type'=>'radio','q_title'=>$title,'qno'=>$fpid,'term'=>$term, 'qop'=>$arr);
			}
			//for age
                        if($fpid == 7){
				$arr=array();
			...
			//for product
                        if($fpid == 12){
                                $arr=array();
                                $sql7="SELECT * from project_sec_map WHERE qset_id=$pid ;";
                                $dq7=DB::getInstance()->query($sql7);
                                if($dq7->count()>0)
                                foreach($dq7->results() as $r7){
                                        $at=$r7->product;
			...
			//for centre
                        if($fpid == 13){
                                $arr=array();
                                $sql7="SELECT * from centre_project_detail WHERE project_id=$pid ;";
                                $dq7=DB::getInstance()->query($sql7);
                                if($dq7->count()>0)
                                foreach($dq7->results() as $r7){
                                        $at='';
...
function get_project_question_term($pid)
{
    ...
    {
	$temp_arr=array();
    	$sql="SELECT distinct a.`q_id`, a.`q_title`, a.`q_type`, a.qno , b.term FROM `question_detail` as a LEFT JOIN question_option_detail as b ON a.q_id = b.q_id WHERE a.q_id IN (select qid from question_sequence WHERE `qset_id`= $pid order by sid) ;";
