...
if(isset($_POST['submit']))
{
  ...
  // $edt=$_POST['edt'];
   ...
   if($qset!=0)
   {
		...
		$tbcq="SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = 'vcims_12052015' AND TABLE_NAME = '$dtable'";
		...
		{	
			...
			if($qset==20)
				//$qr="SELECT distinct resp_id FROM $dtable WHERE q_id=$qset and month(visit_month_$qset)=$sm AND year(visit_month_$qset)=$yrs order by resp_id";
                        ...
                        if($qset==40)
				$qr="SELECT distinct resp_id FROM $dtable WHERE q_id=$qset and month(visit_month_$qset)=$sm  AND year(visit_month_$qset)=$yrs order by resp_id";
                                //$qr="SELECT distinct resp_id FROM $dtable WHERE month(visit_month_$qset)=$sm and resp_id > 727 order by resp_id";
			else
	...
	foreach($rrp->results() as $rp)
		...
		$rdata= r_detail($rsp,$qset,$dtable,$colms,$isd,$sm,$em,$yrs);
		//print_r($rdata);
