...
<center><h3>RFID Analysis Report</h3> </center>
        ...
        //$db1=DB::getInstance()->query("SELECT distinct `brand_name` FROM Digiadmin_rfid.rfid_installed_info WHERE tag_id in (SELECT tagid FROM Digiadmin_rfid.rfid_info) ");

        $db1=DB::getInstance()->query("SELECT distinct `brand_name` FROM Digiadmin_rfid.rfid_installed_info");
        
        if($db1->count()>0)
            foreach($db1->results() as $r)
            { 
                array_push($arr_brands, $r->brand_name);
                echo "<input type=checkbox name=$r->brand_name value=$r->brand_name > $r->brand_name ";
            }
       ...
       // $query = "SELECT distinct  `model_name` FROM Digiadmin_rfid.rfid_installed_info WHERE `tag_id` in (SELECT tagid FROM Digiadmin_rfid.rfid_info)";
 
        $query = "SELECT distinct  `model_name` FROM Digiadmin_rfid.rfid_installed_info";
        ...
        $db1=DB::getInstance()->query($query);
         if($db1->count()>0)
            foreach($db1->results() as $r)
            { 
                array_push($arr_brands, $r->model_name);
                echo "<input type=checkbox name=$r->model_name value=$r->model_name > $r->model_name ";
            }
          else echo 'Not Available';  
        echo "</details>";
        ...
        {   
           ...
           {    
            ...
            if($countb>0)
            {
                foreach($arrb as $b)
                {   if($ctr==0)
                      $strb.="SUM(IF (`brand_name` = '$b',1,0)) as $b ";
                    else
                      $strb.=",SUM(IF (`brand_name` = '$b',1,0)) as $b ";  
                    $ctr++;
                }
            ...
            {   
                ...
                foreach($arrp as $p)
                {   if($ctr==0)
                      $strp.="SUM(IF (`model_name` = '$p',1,0)) as $p ";
                    else
                      $strp.=",SUM(IF (`model_name` = '$p',1,0)) as $p ";    
                    $ctr++;
                }          
           ...
           // $qr.=" FROM Digiadmin_rfid.rfid_installed_info a1 JOIN Digiadmin_rfid.rfid_info a2 ON a1.tag_id = a2.tagid  WHERE tag_id in (SELECT tagid FROM Digiadmin_rfid.rfid_info) AND substring(`timestamp`,1,10) BETWEEN '$fd' AND '$td' group by $dtt ;";

          $qr.=" FROM Digiadmin_rfid.rfid_installed_info WHERE substring(`timestamp`,1,10) BETWEEN '$fd' AND '$td' group by $dtt ;";
          ...
          if($rt=='2')
          {
            ...
            if($countb>0)
            {
                foreach($arrb as $b)
                {   if($ctr==0)
                        {$strb.="COUNT(IF (`brand_name` = '$b',1,NULL)) as $b "; $strbc=" SELECT q1.Date, (@$b := @$b + q1.$b) AS $b"; $strbc1="SET @$b := 0;"; }
                    else
                        { $strb.=",COUNT(IF (`brand_name` = '$b',1,NULL)) as $b "; $strbc.=",(@$b := @$b + q1.$b) AS $b"; $strbc1.="SET @$b := 0;";}
                    $ctr++;
                }
              $qr.=$strb;
            }
            ...
            {   
                ...
                foreach($arrp as $p)
                {   if($ctr==0)
                        {$strp.="COUNT(IF (`model_name` = '$p',1,NULL)) as $p "; $strpc=" SELECT q1.Date, (@$p := @$p + q1.$p) AS $p"; $strpc1="SET @$p := 0;";}
                    else
                        {$strp.=",COUNT(IF (`model_name` = '$p',1,NULL)) as $p "; $strpc.=",(@$p := @$p + q1.$p) AS $p"; $strpc1.="SET @$p := 0;"; }   
                    $ctr++;
                }          
                $qr.=$strp;
            }
            
           // $qr.=" FROM Digiadmin_rfid.rfid_installed_info a1 JOIN Digiadmin_rfid.rfid_info a2 ON a1.tag_id = a2.tagid  WHERE tag_id in (SELECT tagid FROM Digiadmin_rfid.rfid_info) AND substring(`timestamp`,1,10) BETWEEN '$fd' AND '$td' group by $dtt) as q1";
            
          $qr.=" FROM Digiadmin_rfid.rfid_installed_info  WHERE substring(`timestamp`,1,10) BETWEEN '$fd' AND '$td' group by $dtt) as q1";
        ...
        //  $qr="SET @7up := 0; SELECT q1.Date, (@7up := @7up + q1.7up) AS 7up FROM (SELECT substring(timestamp,1,10) as Date, COUNT(IF (`brand_name` = '7up',1,NULL)) as 7up FROM Digiadmin_rfid.rfid_installed_info a1 JOIN Digiadmin_rfid.rfid_info a2 ON a1.tag_id = a2.tagid WHERE tag_id in (SELECT tagid FROM Digiadmin_rfid.rfid_info) AND substring(`timestamp`,1,10) BETWEEN '2015-05-01' AND '2015-07-30' group by substring(timestamp,1,10)) as q1";
